...
- ✅ Календарь (CalDAV) — задачи через `yandex.sh cal task add "Не забыть"`.
...
Форма задачи:
`yandex.sh cal task "Не забыть" add "<summary>" <due-ISO> "источник: <url> / вердикт: <...> / контекст: <...>"`

Автоматический мост (опционально, после П.2/Волны 2): когда `pending_review.json`
получает находку с вердиктом, требующим IRL-действия, автоматом создавать задачу
в «Не забыть». Отдельная Билдер-задача.
